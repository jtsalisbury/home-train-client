 ...
 */
declare enum RuntimeFlag {
    Disabled = -1,
    Off = 0,
    On = 1
}
declare enum RowAddressType {
    /**
...
interface LedMatrix$1 {
    defaultMatrixOptions(): MatrixOptions;
    defaultRuntimeOptions(): RuntimeOptions;
    new (matrixOpts: MatrixOptions, runtimeOpts: RuntimeOptions): LedMatrixInstance;
}
...
type Font$1 = new (name: string, path: string) => FontInstance;
interface LedMatrixAddon {
    isSupported: boolean;
    Font: Font$1;
    LedMatrix: LedMatrix$1;
}
...
interface MappedGlyph extends Glyph {
    x: number;
    y: number;
}
type Word = Glyph[];
...
declare class LedMatrixUtils {
    static encodeMappers(...mappers: PixelMapper[]): string;
}

declare const Font: Font$1;
declare const isSupported: boolean;
declare const LedMatrix: LedMatrix$1;

export { type Color, Font, type FontInstance, type Glyph, GpioMapping, HorizontalAlignment, LayoutUtils, LedMatrix, type LedMatrixAddon, type LedMatrixInstance, LedMatrixUtils, type Line, type MappedGlyph, type MatrixOptions, MuxType, type PixelMapper, PixelMapperType, RowAddressType, RuntimeFlag, type RuntimeOptions, ScanMode, VerticalAlignment, type Word, isSupported };
